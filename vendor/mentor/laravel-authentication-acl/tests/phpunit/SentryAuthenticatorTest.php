...
use Mentordeveloper\Authentication\Classes\SentryAuthenticator;
...
class SentryAuthenticatorTest extends DbTestCase
{
    ...
    /**
     * @test
     * @expectedException \Mentordeveloper\Authentication\Exceptions\AuthenticationErrorException
     **/
    ...
    /**
     * @test
     * @expectedException \Mentordeveloper\Authentication\Exceptions\UserNotFoundException
     **/
    ...
    public function itCanGetActivationToken()
    {
        $mock_user = m::mock('StdClass')
                      ...
                      ->andReturn(true)
                      ->getMock();
        $mock_auth = m::mock('Mentordeveloper\Authentication\Classes\SentryAuthenticator')
                      ->makePartial()
    ...
    {
        $authenticator = m::mock('Mentordeveloper\Authentication\Classes\SentryAuthenticator')
                          ->makePartial()
                          ->shouldReceive('getUser')
                          ->once()
                          ->with($email)
    ...
    {
        $unbanned_user = $this->make('Mentordeveloper\Authentication\Models\User', array_merge($this->getUserStub(), ["banned" => 0]))->first();
