...
use Mentordeveloper\Authentication\Tests\Unit\Traits\UserFactory;
...
class UserControllerTest extends DbTestCase
{
    ...
    public function it_run_signup_and_return_success_on_post_signup()
    {
        ...
        $this->action('POST', 'Mentordeveloper\Authentication\Controllers\UserController@postSignup');

        $this->assertRedirectedToAction('Mentordeveloper\Authentication\Controllers\UserController@signupSuccess');
    ...
    public function it_run_signup_and_return_errors_on_post_signup()
    {
        ...
        $this->assertRedirectedToAction('Mentordeveloper\Authentication\Controllers\UserController@signup');
    ...
    {
        $this->action('GET', 'Mentordeveloper\Authentication\Controllers\UserController@signup');
    ...
    public function itDoesntShowCaptchaOnSignupIfDisabled()
    {
        ...
        $response = $this->action('GET', 'Mentordeveloper\Authentication\Controllers\UserController@signup');
    ...
    public function it_showConfirmationEmailSuccessOnSignup_ifEmailConfirmationIsEnabled()
    {
        ...
        $this->action('GET', 'Mentordeveloper\Authentication\Controllers\UserController@signupSuccess');
    }
    ...
    public function it_show_view_with_success_if_token_is_valid()
    {
        ...
        $this->action('GET', 'Mentordeveloper\Authentication\Controllers\UserController@emailConfirmation',
            '', [
                "email" => $email,
                "token" => $token
            ]);
    ...
    public function it_show_view_with_error_if_token_is_invalid()
    {
        ...
        $token = "_token";
        $mock_service = m::mock('StdClass')->shouldReceive('checkUserActivationCode')->once()->with($email,
            $token)->andThrow(new \Mentordeveloper\Authentication\Exceptions\TokenMismatchException)->shouldReceive('getErrors')->once()->andReturn("")->getMock();
        App::instance('register_service', $mock_service);
    ...
    public function it_show_view_errors_if_user_is_not_found()
    {
        ...
        $token = "_token";
        $mock_service = m::mock('StdClass')->shouldReceive('checkUserActivationCode')->once()->with($email,
            $token)->andThrow(new \Mentordeveloper\Authentication\Exceptions\UserNotFoundException())->shouldReceive('getErrors')->once()->andReturn("")->getMock();
        App::instance('register_service', $mock_service);
    ...
    public function it_show_user_lists_on_lists()
    {
        ...
        $this->action('GET', 'Mentordeveloper\Authentication\Controllers\UserController@getList', [
            "new" => "new input",
    ...
    public function createNewUserWithSuccess()
    {
        ...
        $this->action('POST', 'Mentordeveloper\Authentication\Controllers\UserController@postEditUser', $input_data);
        ...
        $this->assertRedirectedToAction('Mentordeveloper\Authentication\Controllers\UserController@editUser',
            ['id' => $user_created->id]);
    ...
    {
        $user_created = $this->make('Mentordeveloper\Authentication\Models\User', $this->getUserStub());
    ...
    public function canAddCustomFieldType()
    {
        ...
        $user_id = 1;
        $this->action('POST', 'Mentordeveloper\Authentication\Controllers\UserController@addCustomFieldType', ['description' => $field_description, 'user_id' => $user_id]);
        ...
        $this->assertRedirectedToAction('Mentordeveloper\Authentication\Controllers\UserController@postEditProfile', ["user_id" => $user_id]);
        $this->assertSessionHas('message');
    }
    ...
    {
        $user_created = $this->make('Mentordeveloper\Authentication\Models\User', array_merge($this->getUserStub(),["permissions" => ["_perm" => 1]]));
    ...
    public function canDeleteCustomFieldType()
    {
        ...
        $this->action('POST', 'Mentordeveloper\Authentication\Controllers\UserController@deleteCustomFieldType', ["id" => $field_id, "user_id" => $user_id]);
    ...
    {
        $created_user = $this->make('Mentordeveloper\Authentication\Models\User', $this->getUserStub())->first();
        $created_user_profile = $this->make('Mentordeveloper\Authentication\Models\UserProfile', $this->getUserProfileStub($created_user))->first();
    ...
    {
        $mock_logged_user = m::mock('Mentordeveloper\Authentication\Interfaces\AuthenticateInterface')->shouldReceive('getLoggedUser')
                             ->once()
                             ->andReturn($created_user)
                             ->getMock();
        App::instance('Mentordeveloper\Authentication\Interfaces\AuthenticateInterface', $mock_logged_user);
    }
}
