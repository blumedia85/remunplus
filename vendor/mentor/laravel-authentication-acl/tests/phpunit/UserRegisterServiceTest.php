...
class UserRegisterServiceTest extends DbTestCase
{
    ...
    {
        $mock_validator = m::mock('Mentordeveloper\Authentication\Validators\UserSignupValidator')->shouldReceive('validate')->andReturn(true)->getMock();
    ...
    {
        return m::mock('Mentordeveloper\Authentication\Validators\UserSignupValidator')
    ...
    /**
     * @test
     * @expectedException \Mentordeveloper\Authentication\Exceptions\UserExistsException
     **/
    ...
    {
        return m::mock('Mentordeveloper\Authentication\Interfaces\AuthenticateInterface')
    ...
    public function setupActiveStateOfUser()
    {
        $this->disableEmailConfirmation();
        Config::set('laravel-authentication-acl::email_confirmation',false);
        $service = m::mock('Mentordeveloper\Authentication\Services\UserRegisterService');
    ...
    public function checksForActivationAndThrowsUserNotFoundException()
    {
        ...
        {
            $service->checkUserActivationCode($email, $token);
        } catch(\Mentordeveloper\Authentication\Exceptions\UserNotFoundException $e)
        {
            $gotcha = true;
        }
    ...
    {
        $user_stub =
                m::mock('Mentordeveloper\Authentication\Modles\User')->makePartial()->shouldReceive('checkResetPasswordCode')->andReturn(false)->getMock();
        ...
        {
            $service->checkUserActivationCode($email, $token);
        } catch(\Mentordeveloper\Authentication\Exceptions\TokenMismatchException $e)
        {
            $gotcha = true;
        }
    ...
    {
        $user_stub = m::mock('Mentordeveloper\Authentication\Modles\User');
        $user_stub->activation_code = "12345_";
