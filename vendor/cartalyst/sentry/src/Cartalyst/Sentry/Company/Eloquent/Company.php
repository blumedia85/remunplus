<?php namespace Cartalyst\Sentry\Company\Eloquent;
/**
...
use Cartalyst\Sentry\Company\LoginRequiredException;
use Cartalyst\Sentry\Company\PasswordRequiredException;
use Cartalyst\Sentry\Company\CompanyAlreadyActivatedException;
...
class Company extends Model implements CompanyInterface {
	 ...
	 * @var string
	 */
	protected static $groupModel = 'Cartalyst\Sentry\Users\Eloquent\User';
	 ...
	 * @var string
	 */
	protected static $userGroupsPivot = 'users';
	 ...
	 *
	 * @return bool
	 * @throws \Cartalyst\Sentry\Company\LoginRequiredException
	 * @throws \Cartalyst\Sentry\Company\PasswordRequiredException
	 * @throws \Cartalyst\Sentry\Company\CompanyExistsException
	 */
	public function validate()
	{
		...
		$query = $this->newQuery();
		$persistedCompany = $query->where($this->getLoginName(), '=', $login)->first();

		if ($persistedCompany and $persistedCompany->getId() != $this->getId())
		{
			throw new CompanyExistsException("A user already exists with login [$login], logins must be unique for users.");
		}
     ...
     */
    public function invalidateCompanyGroupsCache()
    {
	...
	public function addGroup(GroupInterface $group)
	{
		if ( ! $this->inGroup($group))
		{
			...
			$this->invalidateCompanyGroupsCache();
	 ...
	 * @return void
	 */
	public static function setCompanyGroupsPivot($tableName)
	{
